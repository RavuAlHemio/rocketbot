...
struct Config {
    pub cirrus_json_bz2_paths: Vec<PathBuf>,
...
async fn main() -> ExitCode {
    ...
    for bz2_path in &config.cirrus_json_bz2_paths {
        eprintln!("processing {}", bz2_path.display());

        let bz2_file = File::open(bz2_path)
            .expect("failed to open bz2 file");
        let cirrus_file = DecoderReader::new(bz2_file);
