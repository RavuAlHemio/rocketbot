...
pub(crate) async fn load_plugins(iface: Weak<dyn RocketBotInterface>) -> Vec<Plugin> {
...

    {
        ...
        for (i, plugin_config) in config_guard.plugins.iter().enumerate() {
            ...
            } else if plugin_config.name == "aviate" {
                Box::new(rocketbot_plugin_aviate::AviatePlugin::new(iface_weak, inner_config).await)
            ...
            } else if plugin_config.name == "checkdigit" {
                Box::new(rocketbot_plugin_checkdigit::CheckDigitPlugin::new(iface_weak, inner_config).await)
            ...
            } else if plugin_config.name == "linguistics" {
                Box::new(rocketbot_plugin_linguistics::LinguisticsPlugin::new(iface_weak, inner_config).await)
